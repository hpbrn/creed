...
import { getAgentIconKind, getAgentDisplayName } from "@/lib/agent-icon";
import type { AgentIconKind, McpClient } from "@/lib/creed/creed-data";
...
const isCreedAgent = (name: string) => {
  ...
  return normalized === CREED_AGENT_NAME || /^.+?'s\s+creed$/.test(normalized);
};

type AgentLike =
  | string
  | Pick<McpClient, "name" | "icon">
  | { agentName?: string; icon?: AgentIconKind };
...
function normalizeAgent(agent: AgentLike): {
  ...
  if (typeof agent === "string") {
    return { name: getAgentDisplayName(agent), icon: getAgentIconKind(agent) };
  }

  const name = "name" in agent ? agent.name : (agent.agentName ?? "Unknown");
  return {
    name,
...
function dedupeAgents(agents: AgentLike[]) {
  ...
  return agents.map(normalizeAgent).filter((agent) => {
  ...
  return (
    ...
    >
      {visibleAgents.map((agent, index) => (
        ...
        >
          {isCreedAgent(agent.name) && !agent.icon.startsWith("custom:") ? (
            <CreedAgentGlyph
              className={
                variant === "stacked"
                  ? "h-[64%] w-[64%]"
                  : "h-full w-full scale-[0.82]"
              }
            />
          ) : (
            <IntegrationGlyph
              ...
              assetClassName={
                variant === "stacked"
                  ? "h-full w-full"
                  : "h-full w-full scale-[0.98]"
              }
              iconClassName={cn(
                "h-full w-full",
                agent.icon === "custom" &&
                  (variant === "stacked" ? "scale-[0.78]" : "scale-[0.82]"),
              )}
            />
          )}
        </span>
