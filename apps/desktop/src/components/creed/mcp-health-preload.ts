...
export type McpHealthRange = "7d" | "30d" | "90d" | "all";
...
export function getCachedMcpHealth(
  range: McpHealthRange,
  creedKey = "",
): McpHealthSummary | null {
...
export function loadMcpHealth(
  ...
  if (!entry.promise) {
    entry.promise = fetch(
      `/api/app/mcp/health?range=${range}&creedId=${encodeURIComponent(creedKey)}`,
      { cache: "no-store" },
    )
      .then((response) =>
        response.ok
          ? response.json()
          : Promise.reject(new Error("Could not load MCP health.")),
      )
...
export function preloadMcpHealth(range: McpHealthRange = "all", creedKey = "") {
