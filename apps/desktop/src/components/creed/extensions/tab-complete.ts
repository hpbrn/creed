...
import { aiEnabled, subscribeAiAvailability } from "@/lib/native/ai-availability";
...
export function canInvokeTabComplete(state: EditorState): boolean {
  return (
    state.selection.empty && canInvokeTabCompleteAt(state, state.selection.from)
  );
}
          ...
          apply: (tr, value) => {
            ...
            if (meta) {
              ...
              if (meta.type === "set") {
                if (
                  value.status === "idle" ||
                  value.requestId !== meta.requestId
                ) {
                  return value;
                }
              ...
              if (meta.type === "accept") {
                ...
                if (!remaining)
                  return { ...IDLE, consumed, requestId: value.requestId };
                return {
                  ...value,
            ...
            // A content echo from the file sync keeps the same editor state.
            // It must not cancel an in-flight suggestion, while actual edits,
            // undo, and unrelated document changes still invalidate it.
            if (tr.docChanged)
              return tr.getMeta("creed-content-sync") ? value : IDLE;
        ...
        props: {
          decorations(state) {
            ...
            const widget = Decoration.widget(
              ...
              {
                side: 1,
                key: `tab-ghost-${ghost.requestId}-${ghost.status}-${ghost.text.length}`,
              },
            );
          ...
          handleKeyDown(view, event) {
            ...
            const emptySection = !view.state.doc.textContent.trim();
            if (!emptySection && !canInvokeTabComplete(view.state)) {
              // Returning false here hands Tab to the browser, which moves
              // focus out of the editor and makes ordinary typing feel like
              // it has jumped to another section or control.
              event.preventDefault();
              return true;
            }
            event.preventDefault();
