...
} from "@/lib/creed/section-references";
  ...
  addAttributes() {
    return {
      value: {
        default: "",
        parseHTML: (element) =>
          element.getAttribute("data-tag") ?? element.textContent ?? "",
        ...
        props: {
          handleKeyDown: (view, event) => {
            ...
            if (isSpace && !markType.isInSet($pos.marks())) {
              const textBeforeCursor = $pos.parent.textBetween(
                0,
                $pos.parentOffset,
                "",
                "",
              ...
              tr.setSelection(
                TextSelection.create(tr.doc, from + target.name.length + 1),
              );
              tr.setStoredMarks([]);
              view.dispatch(tr);
            ...
            if (isRight) {
              ...
              if ($pos.pos === range.to) {
                ...
                view.dispatch(
                  state.tr.setStoredMarks(marksWithoutTag(range.to)),
                );
                event.preventDefault();
                return true;
              }
