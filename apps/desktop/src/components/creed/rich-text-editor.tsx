import { registerFlush } from "@/lib/native/workspace";

import type { ComponentType, CSSProperties, ReactNode } from "react";
import { memo, useCallback, useEffect, useMemo, useRef, useState } from "react";
import { createPortal } from "react-dom";
import { createNodeFromContent, Extension, type Editor, type Range } from "@tiptap/core";
...
import { editorContentSync } from "@/lib/editor-content-sync";
import { readEditorDraft, writeEditorDraft } from "@/lib/creed/editor-drafts";
import { restoreDraftFormatting } from "@/lib/native/section-markdown";
import { cn } from "@/components/ui/utils";
import { creedLowlight } from "@/lib/code-highlighting";
import {
  dispatchEditorModifierClick,
  readEditorModifierClick,
} from "@/lib/editor-modifier-click";
...
function normalizeSectionReferenceTags(
  ...
  root
    ...
    .forEach((node) => {
      ...
      const target = findSectionReferenceTarget(rawValue, targets);
...
function RichTextEditorImpl({
    ...
    editorProps: {
      ...
      handleDOMEvents: {
        mousedown: (_view, event) => {
          ...
          const click = readEditorModifierClick(event.target);
          if (!click) return false;
          event.preventDefault();
          event.stopPropagation();
          dispatchEditorModifierClick(click, onOpenSectionRef.current);
          return true;
        },
      },
      handleClick: (_view, _pos, event) => {
        ...
        const click = readEditorModifierClick(event.target);
        if (click?.kind !== "link") return false;
        event.preventDefault();
        event.stopPropagation();
        return true;
      },
  ...
  useEffect(() => {
    ...
    let active = true;
    const documentAtRead = editor.state.doc;
    void readEditorDraft(`${creedId}:${sectionId}`)
      .then((draft) => {
        if (
          ...
          editor.state.doc !== documentAtRead ||
          !draft ||
        ...
        const restored = restoreDraftFormatting(draft.content, content);
  ...
  useEffect(() => {
    ...
    function syncModifier(event: KeyboardEvent | PointerEvent) {
      setModifierLinkMode(event.metaKey || event.ctrlKey);
    }
  ...
  function submitLink() {
    ...
    const href = coerceRichTextHref(linkDraft.trim());
  ...
  useEffect(() => {
    ...
    function syncMobileTabToolbar() {
      ...
      setMobileTabToolbar({
        ...
        y: viewportPageTop + viewportHeight - SELECTION_TOOLBAR_HEIGHT - 8,
      });
    }
  ...
  useEffect(() => {
    ...
    if (normalizedContent === lastEmittedHtmlRef.current) {
      if (editor.isEditable !== (active && !readOnly))
        editor.setEditable(active && !readOnly, false);
      return;
    }
    ...
    if (editor.getHTML() !== normalizedContent) {
      const incoming = createNodeFromContent(normalizedContent, editor.schema, { slice: false });
      const transaction = editorContentSync(editor.state, incoming instanceof ProseMirrorNode
        ? incoming : editor.schema.topNodeType.create(null, incoming));
                  ...
                  {mobileTabToolbar.status === "showing" ? (
                    <>
                      <button
                        ...
                        onClick={() => acceptTabCompletion(editor.view)}
                      >
                        Accept
                      </button>
                      <button
                    ...
                    >
                      ...
                      <DelayedSpinner
                        pending={mobileTabToolbar.status === "loading"}
