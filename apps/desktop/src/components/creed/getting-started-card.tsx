...
// "Get started" checklist. Lives in the bottom-right corner
...
// Writing a custom property on <html> invalidates styles across the document,
// so the root write waits until the reveal transition has finished. Module
// state is safe because only one card exists at a time.
...
export function GettingStartedPresence({
  ...
  return (
    <div
      ...
      className={cn(
        "fixed right-5 bottom-5 z-40 w-[min(356px,calc(100vw-2.5rem))]",
  ...
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" aria-hidden="true">
      ...
      <circle
        ...
        r={radius}
        fill="none"
        stroke={complete ? "var(--creed-success)" : "var(--creed-accent)"}
        strokeWidth="2"
        strokeLinecap="round"
...
// The connected wrapper below feeds the presentational card with real progress.
export function GettingStartedCardView({
  ...
  const doneCount = GETTING_STARTED_STEPS.filter(
    ({ key }) => steps[key],
  ).length;
    ...
    <div className="overflow-hidden rounded-lg border border-[var(--creed-border)] bg-[var(--creed-surface)] shadow-[0_10px_30px_rgba(28,28,26,0.10)]">
      ...
      <div ref={headerRef} className="relative">
        ...
        >
          ...
          {allDone ? null : (
            <span className="absolute right-[4.75rem] top-1/2 -translate-y-1/2 text-[13px] font-medium leading-5 text-[var(--creed-text-primary)]">
              {doneCount}/{total}
            </span>
          )}
        </div>
        {allDone ? (
          ...
          <>
            ...
            >
              <ChevronDown
                className={cn(
                  "h-4 w-4 transition-[rotate] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)]",
                  expanded && "rotate-180",
                )}
              />
            </button>
...
export function GettingStartedCard() {
  const gettingStarted = useCreedStateSelector((state) => state.gettingStarted);
...
function StepRow({
  ...
  // The action link remains a sibling of the checkbox control so it never
