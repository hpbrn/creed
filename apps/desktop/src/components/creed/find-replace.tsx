// Search highlights are viewport overlays so rendering never mutates editor content.
...
import { FindHighlights } from "@/components/creed/find-highlights";
import { FloatingClose } from "@/components/creed/floating-close";
import {
  GripVerticalIcon,
  type GripVerticalIconHandle,
} from "@/components/ui/grip-vertical";
import {
  useCreedActions,
...
export function CreedFindReplace({
  ...
  const ranges = useMemo(() => matches.map((match) => match.range), [matches]);
  ...
  useEffect(() => {
    if (!open || !active || !matches.length) return;
    const match = matches[Math.min(activeIndex, matches.length - 1)];
    match.range.startContainer.parentElement?.scrollIntoView({ block: "center", behavior: "smooth" });
  }, [activeIndex, matches, open, active]);
  ...
  const replaceCurrent = useCallback(() => {
    ...
    const section = state.sections.find(
      (candidate) => candidate.id === match.sectionId,
    );
    if (!section) return;
  ...
  return (
    <>
      {open && active ? <FindHighlights ranges={ranges} activeIndex={Math.min(activeIndex, matches.length - 1)} scrollRef={scrollRef} /> : null}
    <AnimatePresence>
      {open && active ? (
        <motion.div
          drag
          ...
          className="fixed right-6 top-[76px] z-40 overflow-auto w-[360px] max-w-[calc(100vw-3rem)] rounded-[var(--radius-lg)] bg-[var(--creed-surface)] ring-1 ring-foreground/8 shadow-[var(--creed-shadow-popover)]"
        >
          <div className="flex items-center gap-1 p-1">
            ...
            <FloatingClose label="Close find and replace" onClick={close} />
          </div>

          {replaceOpen ? (
            <div className="flex items-center gap-1 border-t border-[var(--creed-border)] p-1 pl-[28px]">
              <input
                value={replacement}
