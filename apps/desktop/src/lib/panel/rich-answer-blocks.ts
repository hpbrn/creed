import { HEADERLESS_TABLE_MARK } from "@/lib/creed/rich-text";
  ...
  while (index < items.length) {
    ...
    while (
      ...
      nodes.push({
        ...
        children: forest(
          items.slice(childStart, index),
          items[childStart]?.indent ?? base + 2,
        ),
      });
    }
...
export function parseAnswerBlocks(markdown: string): AnswerBlock[] {
  ...
  for (const raw of lines) {
    ...
    if (heading) {
      ...
      // The file editor only has h2-h4. A lone `#slug` line is a section tag
      // (no space after #) and never reaches here.
      const level = marks <= 1 ? 2 : marks >= 5 ? 4 : marks;
      blocks.push({
        kind: "heading",
        level,
        text: heading[2],
