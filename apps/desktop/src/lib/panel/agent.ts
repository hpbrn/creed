...
//
// Validator stays a pure module the node test runner can import directly.
// AccentKey / AgentPermission are mirrored as local unions; the editor
// Markdown guide is a local relative import.

import { creedEditorMarkdownGuide } from "../ai/editor-markdown.ts";

export type AgentPermissionValue =
  "read-only" | "propose" | "direct" | "hidden";
...
const SET_PERMISSION_VALUES = new Set<string>([
  "read-only",
  "propose",
  "direct",
]);
...
export function validateAgentActions(
  ...
  for (const entry of raw as RawAgentAction[]) {
    ...
    const permission =
      typeof entry?.permission === "string" ? entry.permission.trim() : "";
    const position =
      typeof entry?.position === "string" ? entry.position.trim() : "";
    const afterSectionId =
      typeof entry?.afterSectionId === "string"
        ? entry.afterSectionId.trim()
        : "";
    ...
    switch (kind) {
      ...
      case "recolor-section": {
        if (!known.sectionIds.has(sectionId) || !ACCENT_SET.has(accent))
          return null;
        actions.push({
          kind,
          sectionId,
          accent: accent as AgentAccentKey,
          reason,
        });
        break;
      }
      case "reorder-section": {
        ...
        if (
          hasAfter &&
          (!known.sectionIds.has(afterSectionId) ||
      ...
      case "set-permission": {
        if (
          !known.sectionIds.has(sectionId) ||
          !SET_PERMISSION_VALUES.has(permission)
        )
          return null;
        actions.push({
          kind,
          sectionId,
          permission: permission as AgentPermissionValue,
          reason,
        });
        break;
      }
...
export function explainInvalidAgentActions(
  ...
  if (raw.length > MAX_ACTIONS)
    return `actions may contain at most ${MAX_ACTIONS} actions`;
  for (let index = 0; index < raw.length; index += 1) {
    ...
    if (kind !== "new-section" && !sectionId)
      return `action ${index + 1} is missing sectionId`;
...
export function validateAgentPlan(
  ...
  if (
    !Array.isArray(value.targetSectionIds) ||
    value.targetSectionIds.length > MAX_ACTIONS
  )
    return null;
  const targetSectionIds = value.targetSectionIds.filter(
    (id): id is string =>
      typeof id === "string" &&
      (known.sectionIds.has(id) || known.archivedIds.has(id)),
  );
  if (targetSectionIds.length !== value.targetSectionIds.length) return null;
  if (value.needsContentDraft !== true && value.needsContentDraft !== false)
    return null;
  ...
  return {
    ...
    reason,
    summary,
    targetSectionIds: [...new Set(targetSectionIds)],
    needsContentDraft: value.needsContentDraft,
    instructions,
  };
}
...
export function buildAgentPlanningPrompt({
  ...
  sections: Array<{
    ...
    name: string;
    content: string;
    agentPermission: AgentPermissionValue;
  }>;
  ...
  return [
    ...
    mentioned.length
      ? `The user explicitly referenced: ${mentioned.join(", ")}.`
      : "",
    "Section catalog:",
    ...sections.map(
      (section) =>
        `[${section.id} | ${section.agentPermission}] ${section.name}: ${fenceSafe(clip(section.content.replace(/\s+/g, " ").trim(), 500))}`,
    ),
    "Archived:",
    ...(archived.length
      ? archived.map((section) => `[${section.id}] ${section.name}`)
      : ["(none)"]),
...
export function agentRequestUsesAnalysis(query: string): boolean {
  return /\b(improve|strengthen|tighten|clarify|clearer|specific|actionable|analysis|score|guidance|gap|critique|better|rewrite|fix)\b/i.test(
    query,
  );
}
...
export function validateAgentSemantics(
  ...
  const names = new Map(
    sections.map((section) => [
      section.name.trim().toLocaleLowerCase(),
      section.id,
    ]),
  );
  const requestedReduction =
    /\b(shorten|condense|summari[sz]e|trim|prune|remove|delete|clear|replace|rewrite from scratch)\b/i.test(
      query,
    );
  for (const action of actions) {
    if (action.kind === "new-section" || action.kind === "rename-section") {
      ...
      if (
        existingId &&
        (action.kind === "new-section" || existingId !== action.sectionId)
      ) {
        return `A section named "${action.name}" already exists.`;
      }
      names.set(
        normalized,
        action.kind === "rename-section"
          ? action.sectionId
          : `new:${normalized}`,
      );
    }
    if (action.kind === "edit") {
      ...
      if (
        !requestedReduction &&
        original.length >= 800 &&
        replacement.length < original.length * 0.6
      ) {
...
export function buildAgentSystemPrompt() {
  return [
    ...
    creedEditorMarkdownGuide("write"),
...
export function buildAgentUserPrompt({
  ...
  const archivedLines = archived.length
    ? archived.map(
        (section) => `[archived section id: ${section.id}] ${section.name}`,
      )
  ...
  return [
    ...
    "Rules:",
    creedEditorMarkdownGuide("write"),
    "- edit and new-section content is the COMPLETE section body in that native Markdown. It renders in the Creed editor, so use tables, checklists, callouts, headings, and the rest when they fit.",
    ...
    ...(analysis.length
      ? [
          ...
          ...analysis.map(
            (item) =>
              `[${item.sectionId} | ${item.score}/100] Gap: ${fenceSafe(item.gap)} Guidance: ${fenceSafe(item.guidance)}`,
          ),
        ]
      : []),
    "<!-- END USER CREED DATA -->",
    ...
    json_schema: {
      ...
      schema: {
        ...
        properties: {
          ...
          reason: {
            type: "string",
            description: "Why the request can't be done (empty when ok).",
          },
          summary: {
            type: "string",
            description: "One short sentence describing what you did.",
          },
          actions: {
            type: "array",
            items: {
