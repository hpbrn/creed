...
import { creedEditorMarkdownGuide } from "../ai/editor-markdown.ts";
...
export type UsageRangeValue = "7d" | "30d" | "90d" | "all";
...
export type PanelSectionReference = {
  id: string;
  name: string;
  accent?: string;
};
...
const USAGE_RANGE_VALUES = new Set<string>(["7d", "30d", "90d", "all"]);
...
const NAVIGATE_TARGETS = new Set<string>([
  "/file",
  "/connections",
  "/settings",
]);
...
export function validatePanelActions(
  ...
  for (const entry of raw as RawPanelAction[]) {
    ...
    switch (kind) {
      case "navigate":
        ...
        actions.push({
          kind,
          target: target as "/file" | "/connections" | "/settings",
        });
        break;
  ...
  // An Ask reply often has no navigation to offer. It is still a valid answer
  // as long as every supplied action is valid; Search decides separately
  // whether an empty plan can satisfy its navigation-only contract.
  return actions;
}
...
export function resolvePanelAnswerReferences(
  ...
  const tokenIds = Array.from(
    normalizedAnswer.matchAll(/\[\[section:([^\]]+)\]\]/g),
    (match) => match[1],
  );
  const hashtagTargets = Array.from(
    normalizedAnswer.matchAll(sectionHashtag),
    (match) => findSectionReferenceTarget(match[2], sections),
  ...
  const namedIds = sections
    .filter(
      (section) =>
        section.name.trim().includes(" ") &&
        normalizedAnswer.includes(section.name),
    )
    .map((section) => section.id);
  const candidateIds = [
    ...supplied,
    ...tokenIds,
    ...hashtagTargets.map((section) => section.id),
    ...namedIds,
  ].filter(
    (value): value is string =>
      typeof value === "string" && knownIds.has(value),
  );
  const references = validatePanelReferences(
    Array.from(new Set(candidateIds)).slice(0, 8),
    sections,
  );
  ...
  let linkedAnswer = normalizedAnswer.replace(
    ...
    (full, lead: string, rawValue: string) => {
      ...
      return target && validIds.has(target.id)
        ? `${lead}[[section:${target.id}]]`
        : full;
    },
  );
  ...
  return {
    answer: linkedAnswer.replace(
      /\[\[section:([^\]]+)\]\]/g,
      (token, id: string) => (validIds.has(id) ? token : "that section"),
    ),
    references,
  };
}
...
const fenceSafe = (text: string) =>
  text.replace(/(BEGIN|END) USER CREED DATA/gi, "$1_USER_CREED_DATA");
...
const CREED_KNOWLEDGE = [
  ...
  "- Creed is a Mac app for a personal context profile stored as a Markdown file on the user's device. Connected agents read it before answering and help keep it current.",
  "- A new file starts with one Identity section. Existing Markdown files keep their content and section names when opened.",
  ...
  "- Editing: the user edits sections directly. Connected agents propose changes or, on direct sections, edit immediately. Proposals are reviewed on the File page and accepted or rejected. Activity records agent changes.",
  "- Connections: AI agents connect locally over MCP while the app is running and appear on the Connections page.",
  "- AI features: Analysis scores how complete, concrete and current the Creed is. Panel has Search, Ask and Agent modes. Tab provides inline suggestions while editing.",
  "- AI features use the user's OpenRouter API key. Model usage shows OpenRouter spend for this Creed over 7, 30, 90 days or all time.",
  "- Settings covers Identity, Agent edit behaviour, Model usage, Archived sections, Data exports and the Danger zone. Deleting a Creed moves its Markdown file to Trash and removes it from the app.",
  "- Creed has no account, billing, managed credits or built-in GitHub sync. The user manages the file and any version control directly.",
...
export function buildPanelSystemPrompt(mode: PanelMode) {
  ...
  return [
    ...
    "Write the shortest complete answer. Use a paragraph when that is enough. Use native editor blocks when the question needs structure or asks you to show a heading, list, checklist, table, callout, code block, or divider.",
    creedEditorMarkdownGuide("answer"),
    "When an answer names a section, use its exact name. Search handles direct navigation, so answer a request to find or open something with the relevant place and a brief direction.",
    ...
    "Return the Markdown answer only. Do not add a JSON envelope or commentary about your process.",
  ].join("\n");
}
...
function renderSharedContext({
  ...
  return [
    ...
    '- Settings sections (kind "settings-section", target): profile (Creed identity), agent-edits, model-usage (OpenRouter key, balance and spend), archived, data, danger.',
    '- Model usage control: kind "usage-range" (value 7d|30d|90d|all; "this week"=7d, "this month"=30d, longer=90d, "all time"/"ever"=all).',
...
export function buildPanelUserPrompt({
  ...
  const examples =
    mode === "search"
      ? [
          ...
          '- "OpenRouter spend this month" -> ok, actions: [{kind: usage-range, value: 30d}]',
          '- "the proposal about pricing" -> ok, actions: [{kind: file-proposal, target: matching id}]',
        ].join("\n")
      : [
          "",
          "Examples:",
          '- "how much have I spent this month?" -> answer says you are opening it, actions: [{kind: usage-range, value: 30d}] (going there IS the point)',
  ...
  return [...knowledge, context, examples, ...request]
    .filter(Boolean)
    .join("\n");
}
...
export function buildAskMessages({
  ...
  const context = buildPanelUserPrompt({
    mode: "ask",
    query,
    page,
    sections,
  ...
  return [
    ...
    ...history.map<PanelChatMessage>((turn) => ({
      role: turn.role,
      content: clip(turn.text, 800),
    })),
      ...
      schema: {
        ...
        properties: {
          ...
          answer: {
            type: "string",
            description:
              "Ask mode: a concise Markdown answer using native Creed editor blocks (headings, lists, checklists, GFM tables, callouts, code, dividers, inline marks). Search mode: empty.",
          },
          references: {
            type: "array",
            description:
              "Section IDs used as #section-id references in the answer. Empty in Search mode.",
            maxItems: 8,
            items: { type: "string" },
          },
          actions: {
