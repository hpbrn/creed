...
// ones that grow only when useful. All ten ship as agent-writable so AI can
// keep the profile
...
export function normalizeLegacyAccent(accent: unknown): AccentKey {
...
export type CreedSwitcherItem = {
  ...
  avatarInitials: string;
  avatarUrl?: string;
  // Leftover rows may still have a non-null onboarding_stage. New Creeds are
  // ready immediately and do not set this.
  needsSetup?: boolean;
};
...
// "Get started" checklist (the card by the toast stack).
...
export type CreedState = {
  ...
  creedId?: string;
  persistenceSettingsBaseline?: {
    requireApproval: boolean;
    versionControl: CreedSettings["versionControl"];
  };
  persistenceBaseline?: Record<
    string,
    {
      revision: number;
      position: number;
...
export function buildVisibleCreedMarkdown(sections: CreedSection[]) {
  ...
  return sections
    ...
    .map((section) => {
      ...
      if (!markdown || section.accent === "mono" || section.accent === "custom")
        return markdown;
...
export function buildHiddenAgentGuidanceMarkdown(options?: {
  ...
  if (tokenizedProposalUrl && options?.proposalToken) {
    lines.push(
      ...
      "- `creed_get_section({ sectionId })` - fetch ONE section in full (id, name, accent, contentMarkdown, contentHtml, lastEditedBy). Use this before update / append.",
...
// still mention `creed.md`. They're shown in copy-prompt previews. Swap
// them to your domain if you want forks
...
export const initialCreedState: CreedState = {
  ...
  connections: [
    ...
    },
    {
      ...
      icon: "grokbot",
      status: "not-connected",
      description: "Connect Grok Bot to Creed through a public MCP tunnel.",
      connectHint:
        "Expose Creed's local MCP URL through a secure public tunnel, then add that URL in Grok Bot's custom connector settings.",
    },
    {
      id: "cursor",
      name: "Cursor",
    ...
    },
    {
      ...
      connectHint:
        "Run opencode mcp add creed with the URL above, then authorize Creed in the browser if OpenCode requests it.",
    },
    {
      id: "devin",
      name: "Devin",
      icon: "devin",
      status: "not-connected",
      description: "Connect Devin to Creed through a public MCP tunnel.",
      connectHint:
        "Expose Creed's local MCP URL through a secure public tunnel, then add it in Devin's MCP Marketplace as an HTTP server.",
    },
    {
      id: "factory",
      name: "Factory",
