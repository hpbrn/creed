...
function withInlineCode(rawText: string, render: (rest: string) => string) {
  ...
  const stashed = rawText.replace(/`([^`\n]+)`/g, (_match, body: string) => {
    const token = `%%CREEDINLINECODE${placeholders.length}%%`;
    placeholders.push(`<code>${escapeHtml(body)}</code>`);
    return token;
  });
  let out = render(stashed);
  placeholders.forEach((html, index) => {
    out = out.replace(`%%CREEDINLINECODE${index}%%`, html);
  });
  return out;
}
...
export function coerceRichTextHref(href: string): string | null {
  ...
  if (/^[a-z0-9][a-z0-9.-]*\.[a-z]{2,}([/:?#].*)?$/i.test(hostish)) {
    return `https://${hostish}`;
  }
...
function applyInlineLinks(escapedText: string) {
  return escapedText.replace(
    ...
    (match, label: string, href: string) => {
      const safeHref = coerceRichTextHref(href);
      if (!safeHref || !/^(https?:|mailto:)/i.test(safeHref)) return match;
...
function wrapMarkdownList(kind: MarkdownListKind, itemsHtml: string) {
  ...
  const listClass =
    kind === "ol"
      ? "creed-list creed-list-ordered"
      : "creed-list creed-list-bullet";
...
export function removeSectionReferences(
  ...
  const targetValues = new Set(
    [target.id, target.name].map(normalizeSectionReference).filter(Boolean),
  );
  let removed = false;
...
export function normalizeRichTextInput(input: {
  ...
  if (
    ...
    return sanitizeRichTextHtml(
      markdownToRichHtml(input.contentMarkdown.trim()),
    );
  }
...
export function looksLikeMarkdown(text: string) {
  ...
  if (/\[[^\]]+\]\((?:https?:|mailto:|\/|#)[^)\s]+\)/.test(trimmed))
    return true;
  return /(\*\*[^*\n]+?\*\*|==[^=\n]+?==|~~[^~\n]+?~~|__[^_\n]+?__)/.test(
    trimmed,
  );
