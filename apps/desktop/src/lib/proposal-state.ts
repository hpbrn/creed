...
import { getProposalPreviewText } from "@/lib/creed/creed-data";

export function proposalActivityEntries(
  ...
  const entries = activity.flatMap((entry) => {
    if (
      entry.status !== "pending" ||
      !entry.proposalId ||
      !stale.has(entry.proposalId)
    ) {
      return [entry];
    }
    if (recorded.has(entry.proposalId)) return [];
    recorded.add(entry.proposalId);
    return [{ ...entry, status: "stale" as const }];
  });
  const represented = new Set(entries.map((entry) => entry.proposalId));
  for (const proposal of proposals) {
    if (proposal.status !== "pending" && proposal.status !== "stale") continue;
    if (represented.has(proposal.id) || recorded.has(proposal.id)) continue;
    represented.add(proposal.id);
    entries.push({
      id: `proposal-${proposal.id}`,
      proposalId: proposal.id,
      createdAt: proposal.createdAt,
      dayLabel: proposal.createdAt
      ...
      actorType: proposal.authorType ?? "agent",
      avatarUrl: proposal.authorAvatarUrl,
      avatarInitials: proposal.authorInitials,
      summary: `${proposal.sectionName} proposal`,
      ...
      afterText: getProposalPreviewText(proposal.draft),
    });
  }
...
export function inlineReviewProposals(proposals: Proposal[]): Proposal[] {
  return pendingReviewProposals(proposals);
}

// New-section proposals stay in live review. Existing-section proposals park
// with their section: hidden while archived, back on restore.
export function proposalTargetsLiveSection(
  proposal: Pick<Proposal, "sectionId" | "draft">,
  sections: ReadonlyArray<Pick<CreedSection, "id" | "archived">>,
) {
  if (
  ...
  return section != null && !section.archived;
}

export function liveReviewProposals(
  proposals: Proposal[],
  sections: ReadonlyArray<Pick<CreedSection, "id" | "archived">>,
) {
  return proposals.filter((proposal) =>
    proposalTargetsLiveSection(proposal, sections),
  );
}
...
export function mergePendingProposalOutcomes(
  ...
  const currentById = new Map(
    current.map((proposal) => [proposal.id, proposal]),
  );
  const incomingById = new Map(
    incoming.map((proposal) => [proposal.id, proposal]),
  );
