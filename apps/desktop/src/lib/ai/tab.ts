...
//
import { creedEditorMarkdownGuide } from "./editor-markdown.ts";
...
export function buildTabSystemPrompt(): string {
  return [
    ...
    "- Choose structure by meaning and local style. Continue an existing list with the same marker. Indent two spaces to nest. Use a callout only for a real warning or governing rule, a table only for aligned fields, code only for literal code or commands, headings only to open a distinct group, and dividers only between substantial groups. Plain prose is the default.",
...
export function sanitizeTabCompletion(raw: string, before: string): string {
  ...
  if (/\s$/.test(before) || before === "") {
    ...
    const punctuationNeedsSpace =
      /[.,!?;:)]/.test(previous) && /[\p{L}\p{N}#]/u.test(next);
    const wordBoundaryNeedsSpace =
      /[\p{L}\p{N}]/u.test(previous) && /[\p{L}\p{N}#]/u.test(next);
    if (hadLeadingWhitespace || punctuationNeedsSpace || wordBoundaryNeedsSpace)
      text = ` ${text}`;
  }
  return text.trimEnd();
}
