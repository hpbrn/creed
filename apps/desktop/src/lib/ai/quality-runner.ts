...
import type { CreedSection } from "@/lib/creed/creed-data";
...
const LOW_BALANCE_THRESHOLD_USD = 0;
...
const activeFullRuns = new Set<string>();
const inFlightSection = new Map<
  string,
...
function freezeSnapshot(): RunnerSnapshot {
  ...
  return Object.freeze({
    report,
    fullRunning: [...activeFullRuns].some((key) =>
...
export function subscribeQualityRunAnnouncements(
  scopeKey: string,
  listener: Listener,
) {
  if (typeof BroadcastChannel === "undefined") return () => {};
...
async function requestQualityJson(
  url: string,
  init: RequestInit,
  timeoutMs = 30_000,
  ...
  try {
    // Bound both headers and body reads without cancelling durable server work.
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timeoutId = setTimeout(() => {
          reject(
            new Error("Could not refresh analysis status. Try again shortly."),
          );
        }, timeoutMs);
      }),
    ]);
...
async function waitForQualityRun(run: QualityRun): Promise<QualityRun> {
  ...
  while (current.status === "queued" || current.status === "running") {
    if (Date.now() >= deadline) {
      throw new Error(
        "Analysis is still running. Its result will appear when it finishes.",
      );
    }
    await new Promise((resolve) => window.setTimeout(resolve, 1_000));
    const result = await requestQualityJson(
      `/api/app/ai/quality?runId=${encodeURIComponent(current.id)}`,
      {
        method: "GET",
        cache: "no-store",
      },
    ...
    const payload = result.payload as { run?: QualityRun; error?: string };
...
async function requestQuality(args: FullRunArgs): Promise<FullRunResult> {
  const result = await requestQualityJson("/api/app/ai/quality", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      creedId: args.scopeKey,
  ...
  const { response } = result;
  const payload = result.payload as FullRunResult & { error?: string };
  if (!response.ok) {
...
export function runFullQuality(args: FullRunArgs): Promise<FullRunResult> {
  ...
  if (!args.readOnly) activeFullRuns.add(key);
    ...
    try {
      ...
      if (!args.readOnly && activeScopeKey === args.scopeKey) {
        ...
        recordOutcome({
          ...
          message: null,
          lowCredits:
            typeof balance === "number" && balance < LOW_BALANCE_THRESHOLD_USD,
        });
      }
      return payload;
    } catch (cause) {
      const message =
        cause instanceof Error
          ? cause.message
          : "Could not analyze Creed quality.";
...
export function runSectionQuality(
  ...
  const promise = (async () => {
    try {
      ...
      return (
        payload.report?.sections.find(
          (entry) => entry.sectionId === args.section.id,
        ) ?? null
      );
    } catch (cause) {
      const message =
