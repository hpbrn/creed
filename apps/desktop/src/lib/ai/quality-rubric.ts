...
import { buildVisibleCreedMarkdown } from "@/lib/creed/creed-data";
...
export function buildQualityPrompt(
  ...
  return [
    ...
    "Your job is to judge how well this file lets a fresh AI know its owner. Be demanding and consistent: the same content must always earn the same score.",
    "",
    ...
    JSON.stringify(
      {
        overall: {
          ...
          strength: {
            ...
            detail:
              "Specifies wake/sleep windows, deep-work mornings, and weekly cadences so AI can plan around them.",
          },
          gap: {
            tag: "Vague",
            title: "Goals stuck in vague mode",
            detail:
              "Goals are aspirational but lack concrete outcomes or stale-by signals so AI can't pull on them.",
          },
          guidance: {
            type: "ask",
            title: "Clarify the priority",
            detail:
              "Ask which active goal should outrank the others this quarter.",
        ...
        sections: [
          {
            ...
            strength: {
              ...
              detail:
                "Says what the user does and where their work happens so AI has a starting frame.",
            },
            gap: {
              tag: "Vague",
              title: "Missing defining traits",
              detail:
                "Describes the user without showing the values or defaults that should change AI replies.",
            },
            guidance: {
              type: "ask",
              title: "Add defining defaults",
              detail:
                "Ask which values or defaults AI should anchor every reply on.",
...
export function buildQualityResponseFormat(): Record<string, unknown> {
  ...
  const guidance = {
    ...
    properties: {
      type: {
        type: "string",
        enum: ["edit", "ask", "remove", "move", "review"],
      },
      title: { type: "string" },
      detail: { type: "string" },
          ...
          sections: {
            ...
            items: {
              ...
              properties: {
                ...
                band: {
                  type: "string",
                  enum: QUALITY_SCORE_BANDS.map((band) => band.key),
                },
                score: { type: "integer" },
                tags: { type: "array", items: { type: "string" } },
                strength: note(tagVocabulary.green),
