...
import { DROPDOWN_CHEVRON_CLASS } from "@/components/ui/dropdown-menu";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
const TAG_DISPLAY_PRIORITY: Record<
...
function QualityEmptyState({
  ...
  return (
    <div className="space-y-3">
      <div className="text-[15px] font-medium leading-none text-[var(--creed-text-primary)] md:text-[16px]">
        {loading ? loadingLabel : emptyLabel}
      </div>
      {actionAvailable && !loading && onAction ? (
        <button
          type="button"
          onClick={onAction}
          className="text-[13px] font-medium text-[var(--creed-accent)] transition-colors hover:text-[var(--creed-accent-hover)]"
        >
          Run analysis
        </button>
      ) : null}
...
function QualityNoteRow({
  ...
  return (
    <div className="group/note rounded-md hover:bg-[var(--creed-surface-raised)]">
      <button
        type="button"
        onClick={() => setOpen((value) => !value)}
        aria-expanded={open}
        className="group flex w-full items-center gap-1.5 px-1.5 py-1 pr-2 text-left"
      >
        <span
          className="shrink-0 font-mono text-[12px] font-medium leading-[1.2]"
          style={{ color: accent, transform: "translateY(-1px)" }}
        >
          {symbol}
        </span>
        ...
        <ChevronDown
          className={cn(
            ...
            "h-3 w-3 group-aria-expanded:rotate-180 group-aria-expanded:text-[var(--creed-text-primary)]",
          )}
        />
      </button>
      <AnimatePresence initial={false}>
        {open ? (
          ...
          >
            <div className="px-1.5 pb-1.5 pl-[18px] text-[12px] leading-[1.5] text-[var(--creed-text-primary)]">
              {note.detail}
            </div>
          </motion.div>
        ) : null}
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          onMouseEnter={isMobile ? undefined : openOnHover}
          ...
          aria-label={
            actionAvailable ? "Run section analysis" : "Section score"
          }
        >
          <QualityRing
      ...
      >
        {quality ? (
          ...
          <QualityEmptyState
            loading={loading}
            loadingLabel="Analyzing this section for agent context..."
            emptyLabel="No section analysis yet."
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      >
        {report ? (
          ...
          <QualityEmptyState
            loading={loading}
            loadingLabel="Analyzing your Creed for agent context…"
