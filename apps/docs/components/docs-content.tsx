...
npm run dev`}</DocsCommand>
        <Steps>
          ...
          <li><p>Enter the owner secret created in <InlineCode>apps/open/.env.local</InlineCode>.</p></li>
          <li><p>Complete Personal onboarding and save your first Creed.</p></li>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <p>
          Setup preserves unrelated environment values, verifies the Supabase keys before saving them, and asks before it changes the database. Generated owner and encryption secrets are written to <InlineCode>apps/open/.env.local</InlineCode> and are never printed. The installer tells you to enter the owner secret from that file once in the browser.
        </p>
        <h3>Manual setup</h3>
...
icacls apps\\open\\.env.local /inheritance:r /grant:r "\${env:USERNAME}:RW"`}</DocsCommand>
        <p>Generate the private secrets on any platform:</p>
        <DocsCommand>{`node -e "console.log(require('node:crypto').randomBytes(48).toString('base64url'))"
  ...
  },
  {
    ...
    content: (
      <>
        <h3>Vercel</h3>
        <Steps>
          <li><p>Fork the repository and import it into Vercel.</p></li>
          <li><p>Set the project root to <InlineCode>apps/open</InlineCode>.</p></li>
          <li><p>Add the production values from <InlineCode>apps/open/.env.example</InlineCode>.</p></li>
          <li><p>Set <InlineCode>NEXT_PUBLIC_SITE_URL</InlineCode> to the final HTTPS origin.</p></li>
          <li><p>Preview and apply Supabase migrations from a trusted local clone.</p></li>
          <li><p>Deploy, claim the installation, then verify a save and one agent connection.</p></li>
        </Steps>
        <h3>Use a personal subdomain</h3>
        <p>If you already use a personal domain, consider hosting Creed at a subdomain such as <InlineCode>creed.example.com</InlineCode>. This avoids another domain and keeps your personal context close to the rest of your digital presence.</p>
        <p>Add the subdomain to Vercel, create the exact CNAME record Vercel requests, and set <InlineCode>NEXT_PUBLIC_SITE_URL</InlineCode> to the custom HTTPS origin. If GitHub version control is enabled, update its OAuth callback URL to use the same origin.</p>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <h3>Owner access</h3>
        <p>The owner secret authorises a browser once. Rotating it invalidates existing owner cookies. Keep the owner and encryption secrets in a password manager.</p>
        <h3>Optional services</h3>
        <p>GitHub OAuth adds manual Markdown push and pull. OpenRouter enables analysis, Panel, Tab, and onboarding composition. The editor and agent connections work without either service.</p>
      </>
    ),
  },
  {
    ...
    content: (
      <>
        <p>When a newer Open release is available, the in-app notice copies the exact pull command. Read the release notes and back up Supabase before updating.</p>
