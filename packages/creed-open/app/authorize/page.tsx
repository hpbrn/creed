...
async function renderAuthorize(params: SearchParams) {
  ...
  if (!user) {
    return (
      <Shell>
        <Message
          title="Authorise this browser"
          body={`Enter your installation owner secret to let ${client.clientName} read and update your Creed.`}
        />
        <div className="mt-6 flex justify-center">
          <Link
            href={`/claim?next=${encodeURIComponent(returnTo)}`}
