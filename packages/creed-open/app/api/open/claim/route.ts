...
async function startOwnerSession(password: string) {
  const supabase = await createSupabaseServerClient();
  const signIn = await supabase.auth.signInWithPassword({
    email: OPEN_OWNER_EMAIL,
    password,
  });
  if (signIn.error || !signIn.data.user) return false;
...
export async function POST(request: Request) {
  ...
  if (!body) {
    return NextResponse.json({ error: "Enter your owner code." }, { status: 400 });
  }

  if (typeof body.secret !== "string" || !verifyOpenOwnerSecret(body.secret)) {
    return NextResponse.json({ error: "That owner code is not valid." }, { status: 401 });
  }

  const password = deriveOpenOwnerPassword();
  // Returning owners already have the current password. Sign in first so the
  // eighth digit does not wait on a readiness RPC or Auth Admin refresh.
  if (await startOwnerSession(password)) {
    return NextResponse.json({ ok: true });
  }

  const readiness = await getOpenDatabaseReadiness();
  if (!readiness.ready) {
    return NextResponse.json(
  ...
  if (!owner) {
    ...
    // Sign-in failed because CREED_OWNER_SECRET rotated. Refresh the hidden
    // Auth password, then open the session with the new derivation.
