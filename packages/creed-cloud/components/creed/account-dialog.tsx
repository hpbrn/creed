...
export function AccountDialog({
  ...
  useEffect(() => {
    ...
    // Granted figures on the settings cache are always the home wallet, even
    // when this Creed is not home. Reuse them so the card does not wait on
    // /credits/home. Hide immediately only when that cache already says there
    // is no renewable allowance.
    if (cached?.allowanceResets) {
  ...
  const selectedBonusCreed =
    ...
    bonusOptions[0] ??
    null;
  const showBonusCard = bonusBalanceLoading || bonusResets;
  const showAllowance =
