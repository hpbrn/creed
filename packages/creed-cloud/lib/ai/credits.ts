...
async function hasPrivateTesterAllowance(userId: string): Promise<boolean> {
  if (!isPrivateCloud()) return false;

  const admin = getSupabaseAdminClient();
  const { data, error } = await admin.auth.admin.getUserById(userId);
  if (error || !data.user) {
    log.warn("credit_tester_access_read_failed", {
      userId,
  ...
  return hasPrivateCloudAccess(data.user.email);
}

// Private testers receive the same monthly allowance without synthetic Stripe
// state. Public Cloud continues to derive allowances only from entitlements.
async function resolveAllowance(userId: string): Promise<Allowance | null> {
  if (await hasPrivateTesterAllowance(userId)) {
    return {
      micro: GRANT_MONTHLY_USD * MICRO_PER_USD,
      periodKey: monthlyAllowancePeriodKey(),
    };
  }
