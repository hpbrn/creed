...
test("owner code rotation invalidates an existing cookie", () => {
  const value = createOpenOwnerCookieValue(firstSecret, now + 60_000);
...
test("owner codes are exactly eight digits", () => {
  assert.equal(isOpenOwnerCode("12345678"), true);
  assert.equal(isOpenOwnerCode("00000001"), true);
  assert.equal(isOpenOwnerCode("1234567"), false);
  assert.equal(isOpenOwnerCode("123456789"), false);
  assert.equal(isOpenOwnerCode("1234567a"), false);
});
