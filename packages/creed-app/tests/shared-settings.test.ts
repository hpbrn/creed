...
test("permission lists exclude archived sections", () => {
  assert.match(settings, /const liveSections = state\.sections\.filter\(\(section\) => !section\.archived\)/);
  assert.match(settings, /liveSections\.map\(\(section\) =>/);
  assert.match(sharedSettings, /const liveSections = state\.sections\.filter\(\(s\) => !s\.archived\)/);
  assert.match(sharedSettings, /liveSections\.map\(\(section\) =>/);
  assert.doesNotMatch(
    settings.split("Per-section permissions")[1] ?? "",
