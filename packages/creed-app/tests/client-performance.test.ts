...
test("shell nav selects the clicked route before the layout refetch", () => {
  const shell = source("../components/creed/shell.tsx");
  const navigation = source("../components/creed/app-navigation.tsx");
  const surface = source("../components/creed/persistent-file-surface.tsx");
  assert.match(navigation, /pending\?\.to \?\? pathname/);
  assert.match(navigation, /setPending\(\{ from: pathname, to: nextPath \}\)/);
  assert.match(navigation, /pathname !== pending\.from/);
  assert.match(navigation, /popstate/);
  assert.match(shell, /onNavigate=\{\(\) => navigate\(item\.href\)\}/);
  assert.match(surface, /useAppPath\(\)/);
  assert.match(surface, /appSurfaceFromPath\(path\)/);
  assert.match(
    shell,
    /SIDEBAR_NAV_PRESS_CLASS =\s*"transform-gpu transition-\[color,transform,filter\]/,
  );
});
...
test("app routes are lazily mounted once and retained by the app shell", () => {
  ...
  assert.match(shellLayout, /<AppNavigationProvider>/);
...
test("active Creed resolution is passed through the app layout", () => {
  ...
  assert.match(layout, /activeCreed=\{readyActive\}/);
