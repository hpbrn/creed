...
test("suite has the intended task mix and unique ids", () => {
  assert.equal(BENCH_TASKS.length, 22);
  assert.equal(new Set(BENCH_TASKS.map((task) => task.id)).size, 22);
  assert.equal(BENCH_TASKS.filter((task) => task.family === "single-tool").length, 8);
  assert.equal(BENCH_TASKS.filter((task) => task.family === "multi-tool").length, 9);
  assert.equal(BENCH_TASKS.filter((task) => task.family === "adversarial").length, 5);
});
...
test("benchmark tool names stay in sync with the production MCP route", () => {
  ...
  const productionBlock = route.slice(
    ...
    route.indexOf("function listToolsFor("),
  );
