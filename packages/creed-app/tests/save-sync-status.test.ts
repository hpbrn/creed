...
test("sync status keeps structural failures distinct from local drafts", () => {
  ...
  const openEdition = source("../../../apps/open/edition/config.ts");
  const cloudEdition = source("../../../apps/cloud/edition/config.ts");
  ...
  assert.match(openEdition, /persistedLabel: "Saved to database"/);
  assert.match(cloudEdition, /persistedLabel: "Synced to cloud"/);
  assert.match(
    openEdition,
    /persistedTone: (?<tone>[^,]+),[\s\S]*?pendingTone: \k<tone>,/,
  );
  assert.match(
    cloudEdition,
    /persistedTone: (?<tone>[^,]+),[\s\S]*?pendingTone: \k<tone>,/,
  ...
  assert.match(
    file,
    /function SaveStatus\(\) \{[\s\S]*?w-max[\s\S]*?whitespace-nowrap/,
  );
});
