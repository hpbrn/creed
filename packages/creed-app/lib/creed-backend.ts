...
/**
 * Cheap "does this user already have a personal Creed row?" probe used by
 * `app/page.tsx` before it provisions a missing one and sends the user to
 * `/file`.
 *
 * The signal is the personal `creeds` row itself, not the section count.
 * A user who deletes or archives every section still has a Creed.
 */
...
async function loadCreedStateImpl(
  ...
  // sections is still a real Creed whose sections were all deleted or
  // archived. It must load and keep persisting as an empty file.
  const readToken = tokenRow.read_token ?? "";
  const proposalToken = tokenRow.proposal_token ?? "";
  const directEditToken = tokenRow.direct_edit_token ?? "";
  const { definitions } = CONNECTION_DEFINITIONS;
