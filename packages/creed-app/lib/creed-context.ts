 ...
 * with no Creed row yet). Callers that need a file provision one with
 * `ensureReadyPersonalCreed`. `client` is the caller's session client (used
 * to read membership under RLS).
 */
...
/**
 * First-run Personal Creed: reuse the existing row, or create one that is
 * already ready for /file, with an Identity section.
 */
export async function ensureReadyPersonalCreed(
  client: unknown,
  user: User,
): Promise<string> {
  const existing = await getPersonalCreedId(client, user.id);
  ...
  if (error || !created) {
    throw new Error("Could not create a Personal Creed.");
  }
  return created.id;
}
