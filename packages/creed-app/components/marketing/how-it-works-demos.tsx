...
// current app:
//  - CreateDemo: types a first Identity line into a starter file.
//  - ConnectDemo: a single "All agents" card with a copy-URL flash.
...
const IDENTITY_LINE = "Founder and designer in Lisbon.";
...
export function CreateDemo() {
  ...
  const typed = useTypedLoop(IDENTITY_LINE, !done);
      ...
      <div className="flex min-h-[232px] flex-col rounded-lg border border-[var(--creed-border)] bg-[var(--creed-surface)] p-4 shadow-[0_8px_24px_rgba(28,28,26,0.04)] lg:min-h-0">
        <div className="h-1 w-full overflow-hidden rounded-full bg-[var(--creed-surface-raised)]">
          <motion.div
            ...
            initial={false}
            animate={{ width: done ? "100%" : `${(typed.length / IDENTITY_LINE.length) * 100}%` }}
        ...
        <AnimatePresence mode="wait" initial={false}>
          {!done ? (
            ...
            >
              ...
              <div className="mt-3 min-h-11 rounded-xl border border-[var(--creed-border)] bg-[var(--creed-surface)] px-3.5 py-3 text-[14px] leading-5 text-[var(--creed-text-primary)]">
                {typed || "\u00A0"}
              </div>
            </motion.div>
          ) : (
            ...
            >
              ...
              <div className="text-[14px] font-medium text-[var(--creed-text-primary)]">Your Creed is ready</div>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
