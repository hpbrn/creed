...
// Outlined secondary action, shared with the MCP/CLI "Show instructions"
// control so those buttons match the per-agent cards.
export const CONNECTION_SECONDARY_BUTTON_CLASS =
  "w-fit max-w-full justify-center rounded-md border border-[var(--creed-border-strong)] bg-[var(--creed-surface)] px-4 text-[var(--creed-text-primary)] hover:bg-[var(--creed-surface-raised)]";
...
export function ConnectionCard({
  ...
  const buttonClass = cn(
    "w-fit max-w-full justify-center rounded-md px-4",
  ...
  const renderAction = (
    ...
    const className =
      slot === "primary" ? buttonClass : CONNECTION_SECONDARY_BUTTON_CLASS;
    if (action.kind === "open") {
