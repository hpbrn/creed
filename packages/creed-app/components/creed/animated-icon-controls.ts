...
export function useLockIconHover(locked: boolean) {
  const lockedIcon = useAnimatedIconControls(120, undefined, 600);
  const unlockedIcon = useAnimatedIconControls(120, undefined, 600);
  return {
    start: locked ? lockedIcon.start : unlockedIcon.start,
    settle: () => {
      lockedIcon.settle();
      unlockedIcon.settle();
    },
    lockedRef: lockedIcon.iconRef,
    unlockedRef: unlockedIcon.iconRef,
  };
}
