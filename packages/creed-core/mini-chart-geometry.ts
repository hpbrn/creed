// Geometry for the mini uptime chart in the account menu's Status panel.
// Kept pure and separate from the component so the arithmetic is testable: both
// tooltip bugs here were arithmetic rather than styling.
...
// and misplaces the tooltip (an "Operational" label is ~99px where a
// "Degraded · 99.71%" label is ~138px, a 19px error at the chart's edge).
export function tooltipCentre(
  ...
  // A tooltip wider than the row cannot satisfy both edges; keep its left edge
  // visible rather than letting it slide off to the right.
