...
- `packages/creed-app/` contains behavior used by both editions: the editor, remaining public inner pages, MCP and HTTP APIs, common routes, AI tooling, and shared application infrastructure.
...
Cloud-only surfaces include login, signup, password reset, billing, Stripe, feedback, invitations, members, managed credits, account deletion, and `/home`. None may exist in the Open route manifest.

Cloud's temporary private-development boundary lives in `packages/creed-cloud/lib/cloud-access.ts` and the Cloud edition auth adapter. `CREED_CLOUD_ACCESS=private` restricts authenticated page, app API, HTTP-token, and MCP access to the server-only comma-separated `CREED_CLOUD_TESTER_EMAILS` allowlist. It disables managed billing, gives allowlisted testers the standard monthly managed-credit allowance without creating Stripe entitlement state, and changes signed-out acquisition actions to `View roadmap`. The mode defaults to `public`, so removing the module wiring and the two variables restores the launch behavior without a data migration.

The shared proxy implementation is `packages/creed-app/proxy.ts`. Each app supplies a compile-time policy. Open requires the owner cookie and redirects an unauthorised root request to `/enter`. Cloud retains hosted account behavior and managed-payment CSP origins.

In-app switches between `/file`, `/connections`, and `/settings` are immediate. The force-dynamic `(creed-app)` layout keeps `usePathname()` on the previous route until it refetches, so the shell holds the click-target path and `PersistentAppSurfaces` shows that surface or its skeleton on the same click. The real navigation still completes in the background.

## Open owner model

Creed Open is a single-owner application with no public login or signup screen.

- The installer provides `CREED_OWNER_SECRET`, an 8-digit owner code, as a server-only environment value.
- The first browser enters it at `/enter`. `/claim` permanently redirects there. Incomplete installations are sent to `/setup`.
...
- Claim attempts are origin-checked and rate-limited.
...
- Another device can be authorised by entering the current owner code.
- API and MCP access continue through their own scoped credentials.

Open fails closed when configuration or migrations are incomplete. `public.creed_schema_version()` gives the service role one bounded readiness check. A valid owner cookie does not skip that check: the app layout sends the owner back to `/setup` until the schema meets `REQUIRED_OPEN_SCHEMA_VERSION`.
...
`CreedState` lives in `packages/creed-core/creed-data.ts`. `CreedProvider` in `packages/creed-app/components/creed/creed-provider.tsx` owns client state and sync. A first-run Personal Creed is created with `create_owned_creed` (one Identity section) and the user enters `/file`. The Get started card is the first-run teaching UI. Open installation-owner claim remains `POST /api/open/claim` and is a different step.

Archiving a section keeps it in state and parks its pending proposals. Live review surfaces filter through `liveReviewProposals` in `packages/creed-app/lib/proposal-state.ts` so parked cards stay out of the editor, review pill, sidebar, panel, and panel navigator until restore. Permanent delete still drops those proposals. Persist does not delete parked rows.
...
Open supports Personal Creeds only. Cloud retains Personal and Shared Creeds while it is developed privately. New Shared Creeds are created ready, the same way Personal Creeds are. Bonus-credit UI follows the owner's subscription allowance in public mode and the standard monthly tester allowance in private mode.
...
The CLI setup card is hidden in both editions until the standalone CLI is rebuilt. Connections shows a full-width MCP card. The reusable agent-card attribution and status UI remains in the shared application for that rebuild.
...
The `/home` landing is composed in `packages/creed-marketing/components/marketing/landing/landing-page.tsx`. Cloud mounts it; Open does not. The hero is a client island. A scroll-tied statement follows, then the remaining sections as siblings in one `main`. Inner marketing pages stay chrome plus a page view in `creed-app`.
...
- The browser setup screen groups readiness into Environment, Database, and Owner access. Raw environment names remain available only as technical details.
- Vercel's system production URL is a valid initial site origin, so a deployment with no user-supplied environment values can build and show `/setup`. Non-Vercel production hosts still require `NEXT_PUBLIC_SITE_URL`.
...
Open update notices compare the installed `@creed/open` package version with stable `open-vX.Y.Z` GitHub releases, including paginated history across other products. Individual commits do not trigger notices. Open-relevant changes in `apps/open`, `packages/creed-open`, or shared dependencies must be included in a versioned Open release to notify installations. The toast opens `https://docs.creed.md/#maintenance`; it does not execute updates.
