...
[Home](https://creed.md) · [Docs](https://docs.creed.md) · [Roadmap](https://creed.md/roadmap) · [Security](./SECURITY.md)
...
The guided installer connects Supabase, creates the private local environment, previews and applies Creed's database, and verifies the installation. Open [http://localhost:3001](http://localhost:3001), enter the 8-digit owner code from setup once, and open the file.
...
Vercel is the recommended way to host Creed. Fork the repository, import it into Vercel, and set the root to `apps/open`. The first deployment can open `/setup` before you add any values. Add the Supabase and owner values from `apps/open/.env.example`, redeploy, then enter the owner code at `/enter`. Applying the database still needs one `npx supabase db push` from a trusted clone.

Host the live app on a subdomain of a personal domain you already use, such as `creed.example.com`. See [SETUP.md](./SETUP.md) for the complete steps.
